...
<div class="rounded-xl border border-neutral-200 bg-white shadow-sm overflow-hidden max-w-4xl">
    <div class="border-b border-neutral-200 px-6 py-4 bg-neutral-50 flex items-center justify-between">
        <h2 class="font-bold text-neutral-800">Editando: {{ $post->titulo }}</h2>
    ...
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="titulo" id="titulo" class="w-full rounded-lg border-neutral-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20" value="{{ old('titulo', $post->titulo) }}" required>
            ...
            <div>
                ...
                <input type="text" name="categoria" id="categoria" placeholder="Ej: Arquitectura, Ingeniería, Sostenibilidad" class="w-full rounded-lg border-neutral-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20" value="{{ old('categoria', $post->categoria) }}" required>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="resumen" id="resumen" rows="2" class="w-full rounded-lg border-neutral-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20" placeholder="Texto corto que aparecerá en la tarjeta de la noticia...">{{ old('resumen', $post->resumen) }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="contenido" id="contenido" rows="10" required class="w-full rounded-lg border-neutral-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20" placeholder="Escribe el contenido completo del artículo aquí...">{{ old('contenido', $post->contenido) }}</textarea>
        ...
        <div class="mb-6 bg-neutral-50 border border-neutral-200 rounded-xl p-4 flex items-start gap-3">
            <div class="flex h-6 items-center">
                <input id="destacado" name="destacado" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" {{ old('destacado', $post->destacado) ? 'checked' : '' }}>
            </div>
        ...
        <div class="mb-8">
            ...
            <div class="rounded-xl border border-neutral-200 bg-neutral-50 p-4">
                <label for="img" class="block text-sm font-medium text-neutral-700 mb-3">Imagen de Portada</label>
                @if($post->img)
                <div class="mb-4">
                    <img src="{{ asset('storage/' . $post->img) }}" alt="Imagen actual" class="h-32 w-auto rounded-lg object-cover shadow-sm">
                </div>
                @endif
